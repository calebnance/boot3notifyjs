...
(function($){
	$.fn.boot3notify = function(options){
		...
		if($('#boot3notify').length){
			...
			$('body').append('<div id="boot3notify" class="alert alert-' + settings.alertClass + ' alert-dismissable hide" style="position: fixed; right: 20px; top: 0;">' + closeBtn + ' ' + settings.msg + '</div>');
		}
		// transition in
